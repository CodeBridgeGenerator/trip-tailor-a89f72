...
const ItinerariesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            userID: _entity?.userID?._id,title: _entity?.title,startDate: _entity?.startDate,endDate: _entity?.endDate,groupSize: _entity?.groupSize,totalBudget: _entity?.totalBudget,pacing: _entity?.pacing,interest: _entity?.interest,createdAt: _entity?.createdAt,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputTextarea id="title" rows={5} cols={30} value={_entity?.title} onChange={ (e) => setValByKey("title", e.target.value)} autoResize  required  />
            </span>
            <small className="p-error">
